...
exports.deleteContact = (req, res, next) => {
    ...
    req.user.removeContact(contactId)
        ...
        .catch(err => {
            res.status(500).json({
                error: "Unable to delete, Try after Sometime"
            });
        })
}
...
exports.sendMsgAll = (req, res) => {
    const msgId = req.body.msgId;
    const msg = req.user.getMsgValue(msgId);
    const contact = req.user.getContact();
    contact.forEach(c => {
        smsApi(c.mobile, msg).then(()=>{
           
        }).catch(err=>{
            console.log(err);  
        })
    })
    res.json("SMS sent to "+contact.length);
}
...
exports.updatePassword = (req, res) => {
    ...
    bcrypt.compare(oldPassword, req.user.password).then(match => {
        if (!match) {
            return res.status(422).json({
                error: "Enter your correct old password"
            });
        }
